...
<option value="{{ $item->kategori_id }}" @if($item->kategori_id == $kategori->kategori_id) selected @endif>{{ $item->kategori_nama }}</option>
@endforeach
</select>
@error('kategori_id')
...
<div class="col-11">
<input type="text" class="form-control" id="kattegori_kode"name="kattegori_kode" value="{{ old('kattegori_kode', $kategori->kattegori_kode) }}" required>
@error('kattegori_kode')
<small class="form-text text-danger">{{ $message }}</small>
@enderror
</div>
</div>
